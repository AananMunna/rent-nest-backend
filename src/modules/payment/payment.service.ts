...
const createPayment = async (payload: any, userId: string, isAdmin: boolean) => {
    ...
    if (rentalRequest.status !== RentalRequestStatus.APPROVED && rentalRequest.status !== RentalRequestStatus.ACTIVE) {
        throw new Error("Payment is only allowed after the rental request has been approved.");
    }
...
const confirmPayment = async (payload: any) => {
    ...
    const status = nextStatus === "FAILED" ? PaymentStatus.FAILED : PaymentStatus.COMPLETED;
